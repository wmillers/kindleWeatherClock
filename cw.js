...
function showIcs(keep){
    ...
    if (tickListFloatIndex==0)
        tickListFloatIndex=addTaskOnPageChange('updateIcsFloatBar();displayReminder();', 2.5*3600*1000);//2.5*2=5<6<2.5*3=7.5
    else
...
function updateIcsFloatBar(){
    var dH=0;
    var t=document.getElementsByClassName('tickList')[0].children[0];
    var tr=document.getElementsByClassName('tickList')[0].getElementsByTagName('table')[0].children[0].children;
    var ti=tickList.important.slice(0,5);
    var baitHeight=0;//tr[0].offsetHeight;
    if (ti.length>0)
    ...
    t.style.display=baitHeight?'block':'none';
